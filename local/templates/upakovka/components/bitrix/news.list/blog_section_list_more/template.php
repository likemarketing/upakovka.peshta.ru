...
?>
<div class="row blog-elements-list blog-elements-list-more">
<?foreach($arResult["ITEMS"] as $key=>$arItem):?>
<?
$this->AddEditAction($arItem['ID'], $arItem['EDIT_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_EDIT"));
$this->AddDeleteAction($arItem['ID'], $arItem['DELETE_LINK'], CIBlock::GetArrayByID($arItem["IBLOCK_ID"], "ELEMENT_DELETE"), array("CONFIRM" => GetMessage('CT_BNL_ELEMENT_DELETE_CONFIRM')));
?>
    <?if($key == 0 && !isset($_REQUEST["PAGEN_1"]) && $arParams["FIRST_ELEMENT"] == "Y"):?>
    <div class="blog-elements-list__item col-12 mb-30px px-mobile-0" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
...
<?else:?>
    <div class="blog-elements-list__item col-sm-4 mb-30px px-mobile-0" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
