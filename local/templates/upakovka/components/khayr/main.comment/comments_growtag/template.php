...
{
    $photoID = CUser::GetByID($arItem["AUTHOR"]["ID"])->Fetch()['PERSONAL_PHOTO'];
    $photoPath = CFile::GetPath($photoID);
	?>
	<div class="stock d-flex">
		<div class="userInfo no-mobile">
            <img src="<?=$photoPath;?>" alt="<?=$arItem["AUTHOR"]["FULL_NAME"]?>" />
		</div>
		<div class="userText">
            <div class="title">
                <img class="on-mobile" src="<?=$photoPath;?>" alt="<?=$arItem["AUTHOR"]["FULL_NAME"]?>" /><?=$arItem["AUTHOR"]["FULL_NAME"]?>
            </div>
			<p>
				<?if ($arItem["MARK"]) {?>
				...
				<?if ($arItem["CAN_MODIFY"]) {?>
					<div class="form comment form_for" id='edit_form_<?=$arItem["ID"]?>'<?=($arResult["POST"]["COM_ID"] == $arItem["ID"] && !$arResult["SUCCESS"] ? " style='display: block;'" : "")?>>
						<form enctype="multipart/form-data" action="<?=$GLOBALS["APPLICATION"]->GetCurUri()?>" method='POST' onsubmit='return KHAYR_MAIN_COMMENT_validate(this);'>
							...
							<input type="submit" value="<?=GetMessage("KHAYR_MAIN_COMMENT_SAVE")?>" style="border-radius:4px;border:1px solid #FA3F3F;background:#fff;color: #FA3F3F;font-size: 12px;"/>
							<a href="javascript:void(0)" onclick='KHAYR_MAIN_COMMENT_back(); return false;' style='margin-top: -19px !important;margin-left: 100px; text-decoration: none;'><?=GetMessage("KHAYR_MAIN_COMMENT_BACK_BUTTON")?></a>
						</form>
					</div>
				<?}?>
...
<div class='khayr_main_comment' id='KHAYR_MAIN_COMMENT_container'>
	...
	<div class="form comment main_form"<?=($arResult["POST"]["PARENT"] > 0 && !$arResult["SUCCESS"] ? " style='display: none;' " : "")?>>
		<?if ($arResult["CAN_COMMENT"]) {?>
			...
			<form enctype="multipart/form-data" action="<?=$GLOBALS["APPLICATION"]->GetCurUri()?>" method='POST' onsubmit='return KHAYR_MAIN_COMMENT_validate(this);'>
                <input type="hidden" name='NONUSER' value='<?=$arResult["POST"]["NONUSER"]?>' <?=($arResult["USER"]["ID"] ? "readonly='readonly'" : "")?> placeholder="<?=GetMessage("KHAYR_MAIN_COMMENT_FNAME")?>" class="w-45" />
				<?if ($arResult["LOAD_AVATAR"]) {?>
					<input type="file" name='AVATAR' value='' placeholder="<?=GetMessage("KHAYR_MAIN_COMMENT_AVATAR")?>" class="w-45" />
				<?}?>
				<?if ($arResult["LOAD_EMAIL"]) {?>
				...
				<?if ($arParams["LEGAL"]) {?>
					...
					<div class="clear pt10"></div>
				<?}?>
				<input type="submit" class="red-button" value="<?=GetMessage("KHAYR_MAIN_COMMENT_ADD")?>" />
			</form>
		<?} else {?>
			<div class="no-auth-mess">
