...
$APPLICATION->SetPageProperty("title", "ПЕШТА - Смена пароля");
...
<?if($arResult["PHONE_REGISTRATION"]):?>
			<tr>
				...
				<td>
					<input type="text" value="<?=htmlspecialcharsbx($arResult["USER_PHONE_NUMBER"])?>" class="bx-auth-input m-0" disabled="disabled" />
			...
			<tr>
				...
				<td><input type="text" name="USER_CHECKWORD" maxlength="50" value="<?=$arResult["USER_CHECKWORD"]?>" class="m-0 bx-auth-input" autocomplete="off" /></td>
			</tr>
<?else:?>
			<tr>
				<td><span class="starrequired">*</span><?=GetMessage("AUTH_LOGIN")?></td>
				<td><input type="text" name="USER_LOGIN" maxlength="50" value="<?=$arResult["LAST_LOGIN"]?>" class="bx-auth-input m-0" /></td>
			</tr>
			<tr>
...
<?endif?>
			<tr>
				...
				<td><input type="password" name="USER_PASSWORD" maxlength="255" value="<?=$arResult["USER_PASSWORD"]?>" class="m-0 bx-auth-input" autocomplete="off" />
...
<?endif?>
    ...
    <input class="auth-section-button__orange mt-2 mb-4" type="submit" name="change_pwd" value="<?=GetMessage("AUTH_CHANGE")?>" />
</form>
...
<p><a class="auth-section-button" href="<?=$arResult["AUTH_AUTH_URL"]?>"><?=GetMessage("AUTH_AUTH")?></a></p>
