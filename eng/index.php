...
$APPLICATION->SetPageProperty("description", "Ролевая печать в Пеште. Контроль качества продукции на пяти этапах производства, современные технологии печати, соблюдение сроков поставки.");
$APPLICATION->SetPageProperty("keywords", "типография, ролевая печать, флексопечать, печать в рулонах");
$APPLICATION->SetPageProperty("title", "Ролевая печать этикеток в Пеште");
...
<div class="vertical-line">
    <div class="red-block-top padding_mobile_left">
        <div class="uk-container uk-container-large uk-section">
            <div>
                <?= GetMessage('Добиваемся результата и превосходим ожидания<br/>благодаря фундаментальным ценностям Пешты:<br/>вариативности, контролю качества и технологиям') ?>
            </div>
        </div>
    </div>
    <div class="uk-container uk-container-large uk-section">
        <div class="uk-margin-medium-bottom uk-margin-medium-top padding_mobile_left" uk-grid>
            <div class="uk-width-1-2@s uk-position-relative">
                <h2 class="fs-36"><?= GetMessage('Вариативность') ?></h2>
                <div class="vertical-line__point-red"></div>
            </div>
            <div class="uk-width-1-2@s fs-18 roboto_font">
                <?= GetMessage('Накопленный опыт и экспертиза помогают нам производить разноплановую продукцию. За счет этого, мы предлагаем вам наиболее выгодный и удобный вариант, полностью отвечающий вашим требованиям.') ?>
            </div>
        </div>
        <?$APPLICATION->IncludeComponent("growtag:news.list", "variativnost", Array(
    ...
    <div class="uk-container uk-container-large">
        <div class=" padding_mobile_left" uk-grid>
            <div class="uk-position-relative">
                <h2><?= GetMessage('Контроль качества') ?></h2>
                <div class="vertical-line__point-red"></div>
            </div>
        </div>
    </div>
    <div class="quality-control uk-section padding_mobile_left">
        <div class="uk-container uk-container-large">
            <?$APPLICATION->IncludeComponent("growtag:news.list", "kontrol_kachestva", Array(
				"ACTIVE_DATE_FORMAT" => "d.m.Y",	// Формат показа даты
				"ADD_SECTIONS_CHAIN" => "N",	// Включать раздел в цепочку навигации
				"AJAX_MODE" => "N",	// Включить режим AJAX
				"AJAX_OPTION_ADDITIONAL" => "",	// Дополнительный идентификатор
				...
				"FILTER_NAME" => "",	// Фильтр
				"HIDE_LINK_WHEN_NO_DETAIL" => "N",	// Скрывать ссылку, если нет детального описания
				"IBLOCK_ID" => "85",	// Код информационного блока
				"IBLOCK_TYPE" => "newpeshta",	// Тип информационного блока (используется только для проверки)
				"INCLUDE_IBLOCK_INTO_CHAIN" => "N",	// Включать инфоблок в цепочку навигации
				"INCLUDE_SUBSECTIONS" => "Y",	// Показывать элементы подразделов раздела
				"MESSAGE_404" => "",	// Сообщение для показа (по умолчанию из компонента)
    ...
    <?$APPLICATION->IncludeComponent("growtag:news.list", "tehnologii", Array(
        "ACTIVE_DATE_FORMAT" => "d.m.Y",	// Формат показа даты
        "ADD_SECTIONS_CHAIN" => "N",	// Включать раздел в цепочку навигации
        "AJAX_MODE" => "N",	// Включить режим AJAX
        "AJAX_OPTION_ADDITIONAL" => "",	// Дополнительный идентификатор
    ...
    <div class="vertical-line__body">
        <div class="vertical-line__point-start"></div>
    </div>
</div>
<?$APPLICATION->IncludeComponent(
	"growtag:main.feedback", 
	"peshta_feedback_index_NEW", 
	array(
		...
		"FORM" => GetMessage('Заявка на тест или пробный заказ'),
		"CATEGORY_ID" => "26",
		"ASSIGNED_BY_ID" => "18",
		"STAGE_ID" => "C26:NEW",
		...
		"COMPONENT_TEMPLATE" => "peshta_feedback_index_NEW",
		"USER_CONSENT" => "N",
		"USER_CONSENT_ID" => "1",
...
<div class="uk-section uk-container uk-container-large uk-margin-medium-top">
    <div class="uk-text-center"><h2 class="fs-36 uk-margin-medium"><?= GetMessage('Решаем любые задачи') ?></h2></div>
	...
	"FIRST_ELEMENT" => "N",
		...
		"PAGER_TITLE" => GetMessage('Блог'),	// Название категорий
		"PARENT_SECTION" => "",	// ID раздела
		"PARENT_SECTION_CODE" => "",	// Код раздела
		"PREVIEW_TRUNCATE_LEN" => "",	// Максимальная длина анонса для вывода (только для типа текст)
		"PROPERTY_CODE" => array(	// Свойства
...
<?$APPLICATION->IncludeComponent("growtag:main.feedback", "peshta_feedback_footer_NEW1", Array(
	"FROM" => "upakovka.peshta.ru",
		...
		"STAGE_ID" => "C26:NEW",
		"USERTYPE_FIELD_LIST_ID" => "540",
		"TITLE_TEXT" => GetMessage('Оставьте заявку на тест или пробный заказ, и наш менеджер свяжется с вами в ближайшие 30 минут, чтобы уточнить детали'),	// Текст заголовка блока
		"USE_CAPTCHA" => "N",
